...
def convertStringList(strList):
    """
    Convert a list of strings to floats or integers if possible
    """
...
class Configaro(object):
    ...
    def __init__(self, name='Configaro'):
        ...
        argParser.add_argument('--ginBindings',dest='ginBindings',type=str,nargs='+',default=[],
                               metavar='key=value',help='changes to be made to the gin configurations')
    ...
    def _parseArgs(self,argParser=None):
        try:
            ...
            for i in range(0,len(addArgsList)):
                ...
                val=True if (val=='true' or val=='True') else (False if (val=='false' or val=='False') else val)
    ...
    def _parseCfgFile(self,cfgFiles,sections=None):
        ...
        for section in sections:
            for key,val in dict(self.cfgParser.items(section)).items():
                listFromStringEnclosedElements = re.findall(r"\"([A-Za-z0-9_=,'().|*${}+/\<>:~!?&%$@` ]+)\"", val)
                if len(listFromStringEnclosedElements) == 0:
                    if ',' in val:
                        logger.debug("Splitting config list for - {0} : {1}".format(key,val))
                        val=convertStringList([e.strip() for e in val.split(",")]) # remove white space after split
                else:
                    val = listFromStringEnclosedElements
                ...
                if str(key) in self.__dict__.keys():
                    ...
                    self.__dict__[key] = self._castType(val)
    ...
    def _castType(self, val):
        floatVal=None
        try:
            floatVal=float(val)
            return int(floatVal) if floatVal.is_integer() else floatVal
        except:
            return val

    def _readParentConfigFiles(self):
        if "includes" in self.cfgParser.sections():
            for key,val in dict(self.cfgParser.items("includes")).items():
                if os.path.isfile(val):
                    print("INFO: Reading included config file '{}'".format(val))
                    self.cfgParser.read(val)
                else:
                    print("ERROR: Could not locate file listed under includes with name " \
                        "'{}'".format(val))
                    sys.exit(0)

    # public methods
    def getReducedConfig(self, stringMatch, removeStringFromKey = True):
        if removeStringFromKey: myTuple = lambda k,v: (k.replace(stringMatch,""), v)
        else: myTuple = lambda k,v: (k, v)
        return dict([ myTuple(key, value) for key, value in self.allConfigKeys if stringMatch.lower() in key.lower() ])
